...
<div x-data="quizTaker({
...
})" x-init="init()" x-cloak class="flex flex-col h-full bg-white dark:bg-gray-900">
    ...
    @if ($showHeader)
        ...
        <div class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-3 sm:px-4 py-2">
            ...
            <div class="w-full dark:bg-gray-700 bg-gray-200 rounded-full h-1.5 mt-2">
                <div class="h-1.5 rounded-full transition-colors duration-300"
                    :style="{
                        'width': (currentQuestionIndex >= 19 ? 100 : ((currentQuestionIndex / totalQuestions) * 100)) + '%',
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                <span class="font-medium text-sm sm:text-base">Q.<span x-text="currentQuestionIndex + 1"></span><span
                        class="sm:inline hidden">/<span x-text="totalQuestions"></span></span></span>
            ...
            <div class="flex items-center gap-1">
                <button @click="togglePause"
                    class="p-2 sm:p-2.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300"
                ...
                <button @click="flagQuestion" class="p-2 sm:p-2.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"
                    :class="isQuestionFlagged ? 'text-yellow-500' : 'text-gray-600 dark:text-gray-300'"
                    title="Flag question">
                    <i class="fas fa-flag"></i>
                </button>
                <button @click="autoAdvance = !autoAdvance"
    ...
    <div class="flex-1 overflow-y-auto p-4 min-h-0">
        <!-- Guest Notice -->
        <div x-show="isGuest" class="mb-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400">
        ...
        <!-- Current Question -->
        <div class="mb-6">
            <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-100" x-text="currentQuestion.text"></h2>
            ...
            <div class="space-y-3" :class="{ 'answer-locked': isAnswerSubmitted }">
                <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                    <div>
                        <input type="radio" :id="'option-' + option.id" :name="'question-' + currentQuestion.id"
                            :value="option.id" x-model="selectedOption" :disabled="isAnswerSubmitted"
                            class="hidden peer" @change="handleOptionSelect(option)">
                        <label :for="'option-' + option.id"
                            class="answer-option flex items-center p-4 border rounded-lg cursor-pointer transition-colors"
                            :class="{
                                'border-blue-500 bg-blue-50 dark:bg-blue-900/20': selectedOption === option.id && !
                                    isAnswerSubmitted,
                                'border-green-500 bg-green-50 dark:bg-green-900/20': isAnswerSubmitted && option
                                    .is_correct,
                                'border-red-500 bg-red-50 dark:bg-red-900/20': isAnswerSubmitted && selectedOption ===
                            ...
                            <div class="flex items-center h-5">
                                <div class="w-5 h-5 rounded-full border flex items-center justify-center"
                                    :class="{
                                        ...
                                        'border-red-500 bg-red-500': isAnswerSubmitted && selectedOption === option
                                            .id && !option.is_correct,
                                        'border-gray-300 dark:border-gray-500': !selectedOption || (selectedOption !==
                                    ...
                                    <svg x-show="isAnswerSubmitted && option.is_correct" class="w-3 h-3 text-white"
                            ...
                            <div class="ml-3 text-sm">
                                <p class="font-medium text-gray-900 dark:text-gray-100" x-text="option.text"></p>
                            </div>
                        </label>
                ...
                <div x-show="showFeedback" class="mt-4 p-4 rounded-lg"
                    :class="{
                        ...
                        'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800': !isAnswerCorrect &&
                            isAnswerSubmitted
                    }">
                    <div x-show="isAnswerCorrect" class="flex items-center text-green-700 dark:text-green-300">
    ...
    <div class="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
        <div class="flex justify-between items-center">
            ...
            <button @click="nextQuestion" 
                ...
                <svg class="w-4 h-4 ml-1 -mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="showLoginModal" x-cloak
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Sign Up Required</h3>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <a href="{{ route('register', ['redirect' => url()->current()]) }}"
                    class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 mr-2">
                    Sign Up
                </a>
    ...
    <!-- Results Modal -->
    <div x-show="showResultsModal" x-cloak
        class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div x-show="xpAnimation.show" x-transition:enter="transition ease-out duration-500"
                ...
                :class="{ 'bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-300 dark:border-yellow-700': xpAnimation
                        .points >=
                        50, 'bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-300 dark:border-blue-700': xpAnimation
                        .points >= 30 && xpAnimation.points <
                        50, 'bg-green-50 dark:bg-green-900/20 border-2 border-green-300 dark:border-green-700': xpAnimation
                        .points < 30 }">
                <div class="flex items-center justify-center space-x-3">
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute -bottom-2 -left-2 text-xl animate-ping" style="animation-delay: 0.5s">🟢
                        </div>
                    </div>
                    <!-- XP Points Counter -->
                    <div
                ...
                <div class="absolute inset-0 overflow-hidden opacity-20 pointer-events-none">
                    <template x-for="i in 20" :key="i">
                        <div class="absolute w-2 h-2 rounded-full"
                            :class="['bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-purple-500', 'bg-pink-500'][Math.floor(
                                Math.random() * 5)]"
                            :style="`
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('quizTaker', (config) => ({
                ...
                init() {
                    ...
                    if (localStorage.getItem(`quiz_${this.quizId}_answers`)) {
                    ...
                    if (localStorage.getItem(`quiz_${this.quizId}_flagged`)) {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (contentType && contentType.includes('application/json')) {
                                ...
                                this.hasPlan = data.length > 0; // Has active subscription if array is not empty
                            } else {
