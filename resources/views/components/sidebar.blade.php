...
<div class="h-full flex flex-col">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        @foreach($navigation as $item)
            ...
            <div class="space-y-1">
                <a 
                    href="{{ $href }}" 
                    class="group flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-150 {{ ($isActive || $hasActiveChild) ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white' }}"
                    @if(!$routeExists) title="Coming soon" @endif
                    @if($item['is_external'] ?? false) target="_blank" @endif
                >
                    <div class="flex items-center">
                        <svg 
                            class="w-5 h-5 mr-3 flex-shrink-0 {{ ($isActive || $hasActiveChild) ? 'text-blue-700 dark:text-blue-300' : 'text-gray-400 dark:text-gray-500' }}" 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                        >
                    ...
                    @if($item['is_external'] ?? false)
                        <svg class="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                    @elseif(isset($item['children']))
                        <svg class="w-4 h-4 transform transition-transform duration-150 {{ ($isActive || $hasActiveChild) ? 'rotate-0 text-blue-700 dark:text-blue-300' : 'rotate-90 text-gray-400 dark:text-gray-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if(isset($item['children']))
                    <div class="ml-8 space-y-1">
                        @foreach($item['children'] as $child)
                            ...
                            <a 
                                href="{{ $childHref }}" 
                                class="block px-4 py-2 text-sm rounded-md transition-colors duration-150 {{ $isChildActive ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white' }}"
    ...
    @auth
        ...
        @can('isAdmin')
            <!-- Admin Portal Link -->
            <a href="{{ route('admin.portal', ['locale' => $currentLocale]) }}" 
               class="flex items-center w-full px-4 py-3 text-sm font-medium text-yellow-700 dark:text-yellow-400 rounded-lg hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors duration-150 mb-2">
                <svg class="w-5 h-5 mr-3 text-yellow-600 dark:text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @endcan
            ...
            <form method="POST" action="{{ route('logout', ['locale' => $currentLocale]) }}" class="w-full">
                ...
                <button 
                    type="submit"
                    class="flex items-center w-full px-4 py-3 text-sm font-medium text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150"
                >
                    <svg 
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 p-4 mt-auto">
            <div class="space-y-3">
                ...
                <div class="text-center">
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('auth.already_registered') }}</p>
                    <a href="{{ route('login', ['locale' => app()->getLocale()]) }}" class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
                        {{ __('auth.sign_in') }}
                    </a>
                </div>
            </div>
