...
<div
    ...
    @if ($isLocked)
        <!-- Lock Overlay -->
        <div class="absolute inset-0 bg-gradient-to-b from-blue-900/20 via-blue-900/10 to-white/90 dark:from-blue-100/20 dark:via-blue-100/10 dark:to-slate-900/90 z-20 flex flex-col items-center justify-end rounded-2xl">
            <!-- Lock Section -->
            <div
                class="w-full bg-white dark:bg-slate-800 rounded-xl p-4 shadow-2xl border border-slate-200 dark:border-slate-700 group-hover:border-blue-500 dark:group-hover:border-blue-600 transition-colors duration-300"
        ...
        @if ($showDashboardFeatures)
            ...
            <div class="p-5 bg-white dark:bg-gray-800 flex-shrink-0">
                <div class="flex justify-between items-center">
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold flex-shrink-0 ml-3 {{ $status === 'completed'
                            ? 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300'
                            : ($status === 'in_progress'
                                ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300'
                                : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300') }}">
                        <span
                            class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $status === 'completed' ? 'bg-green-500' : ($status === 'in_progress' ? 'bg-blue-500' : 'bg-gray-500') }}"></span>
                        @if ($status === 'completed')
                            {{ __('dashboard.quizzes.completed') }}
                        @elseif($status === 'in_progress')
                            {{ __('dashboard.quizzes.in_progress') }}
