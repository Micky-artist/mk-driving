                    ...
                    <div class="p-6">
                        @if ($user->subscriptions->isNotEmpty())
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    {{ strtolower($subscription->status) === 'active' || $isActive ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400' }}">
                                ...
                                <div class="space-y-2 text-sm">
                                    @if ($subscription->plan)
                                        <div class="flex justify-between">
                                            ...
                                            <span
                                                class="font-medium text-gray-900 dark:text-white">{{ number_format($subscription->plan->price, 0) }}
                                                RWF{{ $subscription->plan->billing_interval }}</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <span
                                            class="font-medium text-gray-900 dark:text-white">{{ $subscription->created_at->format('M j, Y') }}</span>
                                    </div>

                                    @if ($subscription->ends_at)
                                        <div class="flex justify-between">
                                            <span
                                                class="text-gray-500 dark:text-gray-400">{{ $subscription->ends_at->isFuture() ? 'Renews' : 'Ended' }}:</span>
                                ...
                                <div class="pt-4 space-y-2">
                                    @if (strtolower($subscription->status) === 'pending')
                                        <form action="{{ route('admin.subscriptions.approve', $subscription) }}"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="w-full action-btn inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-all duration-200">
                        ...
                        @else
                            <div class="text-center py-6">
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">This user doesn't have an active
                                    subscription.</p>
                            </div>
                        @endif
                    </div>
