...
<div id="resultsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 modal-backdrop flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            @else
            <button type="button" onclick="showResetRestriction()" class="flex-1 px-6 py-3 bg-gradient-to-r from-gray-100 to-gray-50 text-gray-700 font-semibold rounded-lg hover:from-gray-200 hover:to-gray-100 border border-gray-200 transition-all transform hover:-translate-y-0.5 shadow-sm hover:shadow">
                {{ __('quiz.retakeQuiz') }}
            </button>
            @endif
        ...
        @if(!$canRetake && !$hasActiveSubscription && $nextRetakeTime)
            <div class="mt-6 bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-blue-500 p-4 rounded-lg">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-4">
                            <a href="{{ url(app()->getLocale() . '/plans') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700">
                                {{ __('quiz.upgradeNow') }}
                            </a>
                        </div>
                    </div>
...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('resetQuiz').addEventListener('click', function(e) {
            // Check if user has an active subscription or can retake
            @if(!$hasActiveSubscription && !$canRetake && $userAttempts > 0)
                e.preventDefault();
                showResetRestriction();
                return false;
            @else
...
function submitQuizForm(isTimeUp = false) {
    ...
    .catch(error => {
        ...
        // Function to show reset restriction popup
        window.showResetRestriction = function() {
            Swal.fire({
